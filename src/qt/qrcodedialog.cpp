...
{
    QString ret = QString("darktoken:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DT unit input in lnReqAmount, we generate the URI with DT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(darktokenUnits::format(darktokenUnits::DT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
